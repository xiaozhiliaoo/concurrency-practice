package org.example.imagestreamgang;

import org.example.imagestreamgang.filters.Filter;
import org.example.imagestreamgang.filters.GrayScaleFilter;
import org.example.imagestreamgang.filters.NullFilter;
import org.example.imagestreamgang.streams.*;
import org.example.imagestreamgang.utils.Options;
...
public class ImageStreamGangTest {
    ...
    private static void runTests() {
        ...
        for (TestsToRun test : TestsToRun.values()) {
            ...
            // obtain and process.
            Iterator<List<URL>> urlIterator = Options.instance().getUrlIterator();
            ...
            ImageStreamGang streamGang = makeImageStreamGang(mFilters, urlIterator, test);
    ...
    private static ImageStreamGang makeImageStreamGang(Filter[] filters,
        ...
        switch (choice) {
            case SEQUENTIAL_STREAM:
                return new ImageStreamSequential(filters, urlIterator);
            case PARALLEL_STREAM:
                return new ImageStreamParallel(filters, urlIterator);
            case COMPLETABLE_FUTURES_1:
                return new ImageStreamCompletableFuture1(filters, urlIterator);
            case COMPLETABLE_FUTURES_2:
                return new ImageStreamCompletableFuture2(filters, urlIterator);
        }
        return null;
    }
